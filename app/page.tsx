...
export default function Home() {
  return (
    <div className="h-full w-full overflow-hidden">
      ...
      <p className="sr-only">
        Where to dance salsa and bachata in Boston. Find Latin dance socials, parties, and classes
        happening tonight, this weekend, and all week. Updated daily.
      </p>
      <HomeJsonLd />
      <MapView />
      <section className="sr-only" aria-label="Event listings">
        ...
        {events.map(e => {
          const desc = cleanDisplayText(e.description).replace(/\s+/g, ' ').slice(0, 200).trim();
          ...
          return (
            <article key={e.id}>
              ...
              {e.styles.length > 0 && <p>Styles: {e.styles.join(', ')}</p>}
              {e.cost && <p>{e.cost}</p>}
              {desc && <p>{desc}</p>}
            </article>
          );
