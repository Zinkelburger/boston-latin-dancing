...
from event_store import classify, paths, slugs
...
def test_slugify_is_ascii_lowercase_with_id_suffix():
    assert slugs.slugify("Noche de Salsa & Bachata!", "ABCDEFGH-1234") == "noche-de-salsa-bachata-abcdefgh"


def test_slugify_strips_accents_instead_of_dropping_words():
    assert slugs.slugify("Fiesta Caribeña", "12345678abc").startswith("fiesta-caribena-")


def test_slugify_caps_the_name_part_at_60_chars():
    slug = slugs.slugify("x" * 200, "12345678")
...
def test_validate_event_accepts_a_complete_event():
    assert classify.validate_event(_valid()) == []


def test_validate_event_reports_every_missing_required_field():
    issues = classify.validate_event({"lat": 1, "lng": 1, "styles": ["salsa"]})
...
def test_validate_event_geocodes_when_coordinates_are_missing(monkeypatch):
    monkeypatch.setattr(su, "geocode", lambda location: (42.37, -71.10))
    ev = {**_valid(), "lat": None, "lng": None}
    assert classify.validate_event(ev) == []
...
def test_validate_event_flags_an_ungeocodable_location(monkeypatch):
    ...
    ev = {**_valid(), "lat": None, "lng": None}
    assert classify.validate_event(ev) == ["could not geocode location"]
...
def test_validate_event_skips_geocoding_for_unknown_venues(monkeypatch):
    ...
    monkeypatch.setattr(su, "geocode", boom)
...
def test_validate_event_flags_undetectable_styles():
    ...
    assert classify.validate_event(ev) == ["styles=other (could not auto-detect)"]
...
def test_dismiss_removes_from_rejected_and_logs(store):
    ...
    assert "dismiss_rejected" in paths.CHANGELOG.read_text(encoding="utf-8")
